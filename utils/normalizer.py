...
class Normalizer:#(nn.Module):
    ...
    def __init__(self): #, accum_rate=0.99):
        super().__init__()

        self.n = 0
        self.accum_mean = None
        self._accum_S = None
        self.accum_std = None
    ...
    def warmup(self, batch: torch.tensor):
        ...
        self.accum_mean = torch.mean(batch, 0)
        self.accum_std = torch.std(batch, 0)
    ...
    def __iadd__(self, value: torch.tensor):
        ...
        if self.accum_mean is None:
            ...
            self.accum_mean = torch.clone(value)

            self._accum_S = torch.clone(value)

            self.accum_std = torch.clone(value)

        else:

            old_mean = torch.clone(self.accum_mean)

            self.accum_mean = old_mean + (value - old_mean) / self.n

            self._accum_S += (value - old_mean) * (value - self.accum_mean)

            self.accum_std = torch.sqrt(self._accum_S / self.n)
